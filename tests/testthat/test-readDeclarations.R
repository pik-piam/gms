context("read declarations test")

example <- c("*** SOF ./core/declarations.gms", 
             "", 
             ...
             "", 
             "pm_eta_conv(tall,all_regi,all_te)                               \"Time-dependent eta for technologies that do not have explicit time-dependant etas, still eta converges until 2050 to dataglob_values. Unit: efficiency (0..1, except for tnrs)\"", 
             "", 
             "pm_EN_demand_from_initialcap2(all_regi,all_enty)                \"PE demand resulting from the initialcap routine. Unit: EJ, except for Uranium, where it is MT U3O8\"", 
             "pm_budgetCO2eq(all_regi)                                        \"budget for regional energy-emissions in period 1\"", 
             ";", 
             "", 
             "*** EOF ./core/declarations.gms")

expected_result <- structure(c("pm_taxCO2eq", "pm_gdp_gdx", "p_inv_gdx", "pm_eta_conv", 
                               ...
                               "Time-dependent eta for technologies that do not have explicit time-dependant etas, still eta converges until 2050 to dataglob_values. Unit: efficiency (0..1, except for tnrs)", 
                               "PE demand resulting from the initialcap routine. Unit: EJ, except for Uranium, where it is MT U3O8", 
...
test_that("all parameters in example are properly detected", {
  expect_identical(readDeclarations(example),expected_result)
})
